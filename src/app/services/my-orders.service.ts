...
export class MyOrdersService {
  ...
  myCart:any = new BehaviorSubject([])
  ...
  get_product_quantity(productID:any){
    let qte = 0
    for (let i =0; i<this.myCart.value.length; i++){
  ...
  removeProductFromOrder(indice:any) {
    let data = this.myCart.value
    data.splice(indice, 1);
    this.myCart.next(data)
    this.calculate_quantity()
  }
  increaseOrderQuantity(indice:any){
    let data = this.myCart.value
    data[indice].quantite =data[indice].quantite + 1
    this.myCart.next(data)
    this.calculate_quantity()
  }
  decreaseOrderQuantity(indice:any){
    let data = this.myCart.value
    data[indice].quantite =data[indice].quantite - 1
    this.myCart.next(data)
    this.calculate_quantity()
