...
export class ResponsableSettingsPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      quartiers: this.fb.array([this.createQuartier()]) as FormArray
    });
  ...
  get quartiers(): FormArray {
    return this.form.get('quartiers') as FormArray;
  }
  ...
  getProductsByCategorie2(id:any) {
    this.products2 = []
  ...
  setResponsableVendeur(data:any) {
    return new Promise((resolve, reject) => {
  ...
  // responsableChanged(responsable){
  //   this.responsableService.getVendeurByResponsable({responsable}).subscribe((vendeurs) => {
  //     this.vendeurs = vendeurs
  //   })
  // }
  getQuartierByVille(ville:any) {
    if (ville) {
      let data = {
        ville
      }
      this.villesService.getQuartierByVille(data).subscribe((quartiers) => {
        this.quartiers1 = quartiers
      })
    }
  }
  ...
  callSetVendeurDayZone(data:any) {
    return new Promise((resolve, reject) => {
